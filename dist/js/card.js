...
Nova.booting(function (Vue, router, store) {
    Vue.component('cache-config', __webpack_require__(2));
});
    ...
    methods: {
        updateConfig: function updateConfig() {
            ...
            Nova.request().post('/nova-vendor/cache-config', {
                ...
                _this.$toasted.show('Config Updated Successfully !!', { type: 'success' });
            }).catch(function (error) {
                if (error.response.status === 422) {
                    _this.$toasted.show('Oops!! Validation Failed', { type: 'error' });
                } else {
                    _this.$toasted.show('Oops!! Failed to Update Config', { type: 'error' });
                }
            });
        },
        resetConfig: function resetConfig() {
            ...
            var vm = this;
            Nova.request().delete('/nova-vendor/cache-config', { data: {
                    'key': this.key,
                    'default': this.defaultValue
                } }).then(function (response) {
                _this2.$toasted.show('Config Reset Successfully !!', { type: 'success' });
                vm.currentValue = vm.defaultValue;
            }).catch(function (error) {
                _this2.$toasted.show('Oops!! Config to Reset', { type: 'error' });
            });
        }
    },
...
var render = function() {
  ...
  return _c(
    ...
    {
      staticClass:
        "card card relative metric px-6 py-4 relative card-panel card-panel"
    },
    [
      ...
      _c(
        "button",
        {
          staticClass: "btn btn-default btn-danger mt-4",
          on: {
            click: function($event) {
